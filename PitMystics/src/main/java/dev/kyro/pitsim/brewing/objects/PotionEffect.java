...
public class PotionEffect implements Listener {
    ...
    public PotionEffect(Player player, BrewingIngredient potionType, BrewingIngredient potency, BrewingIngredient duration) {
        ...
        if(potionType instanceof SpiderEye) AOutput.send(player, "&5&lPOTION! &7Effected with " + potionType.color + potionType.name + " " +
                AUtil.toRoman(potency.tier));
        else AOutput.send(player, "&5&lPOTION! &7Effected with " + potionType.color + potionType.name + " " +
                AUtil.toRoman(potency.tier) + " &7for &f" + Misc.ticksToTime(ticksLeft));

        if(potionType instanceof SpiderEye) potionType.administerEffect(player, potency, getTimeLeft());
    }
    ...
    public void onExpire() {
        if(!(potionType instanceof SpiderEye)) AOutput.send(player, "&5&lPOTION! " + potionType.color + potionType.name + " &7has expired");
