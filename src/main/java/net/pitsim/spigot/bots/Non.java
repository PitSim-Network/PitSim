...
public class Non {
	...
	public void tick() {
		...
		if(npc.isSpawned() && non.getLocation().getY() <= NonManager.SPAWN_Y_LEVEL - 0.1) {
			Location teleportLoc = non.getLocation().clone();
			teleportLoc.setY(NonManager.SPAWN_Y_LEVEL + 1.2);
...
//				npc.getNavigator().setTarget(target, true);
			...
			if(count % 20 == 0) {
				Location midLoc = NonManager.MID_LOCATION;
				...
				if(distanceFromMid >= NonManager.MAX_DISTANCE_FROM_MID) {
					...
					non.teleport(NonManager.SPAWN_LOCATION);
				}
			}
		} else respawn(false);
	...
	public void setDisabled(Boolean toggled) {
		Location spawnLoc = NonManager.SPAWN_LOCATION;
