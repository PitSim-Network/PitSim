...
public class LoadSkinCommand implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if(args.length < 1) {
			player.sendMessage(ChatColor.translateAlternateColorCodes('&', "&c&lERROR!&7 Usage: /" + label + " <skinName>"));
			return false;
		}
	...
	public static void loadTexture(Player player, String skinName) {
		player.sendMessage("Attempting to load texture information for: " + skinName);
		...
		SkinManager.loadAndSkinNPC(skinName, new BukkitRunnable() {
			...
			public void run() {
				...
				player.sendMessage(ChatColor.translateAlternateColorCodes('&', "&9&lSKIN!&7 Printed out skin information to console"));
