...
public class SmokeNormalParticle extends PitParticle {
	public SmokeNormalParticle() {
	}

	public SmokeNormalParticle(boolean accountForPitch, boolean accountForYaw) {
	...
	public void display(EntityPlayer entityPlayer, Location location, ParticleColor particleColor) {
		entityPlayer.playerConnection.sendPacket(new PacketPlayOutWorldParticles(
				EnumParticle.SMOKE_NORMAL, true, (float) location.getX(), (float) location.getY(), (float) location.getZ(),
				0, 0, 0, 0, 0
		));
	}
}
