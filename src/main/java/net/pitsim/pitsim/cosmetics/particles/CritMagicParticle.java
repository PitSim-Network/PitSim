...
public class CritMagicParticle extends PitParticle {
	public CritMagicParticle() {
	}

	public CritMagicParticle(boolean accountForPitch, boolean accountForYaw) {
	...
	public void display(EntityPlayer entityPlayer, Location location, ParticleColor particleColor) {
		entityPlayer.playerConnection.sendPacket(new PacketPlayOutWorldParticles(
				EnumParticle.CRIT_MAGIC, true, (float) location.getX(), (float) location.getY(), (float) location.getZ(),
				0, 0, 0, 0, 0
		));
	}
}
