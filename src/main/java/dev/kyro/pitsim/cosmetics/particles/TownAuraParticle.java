...
public class TownAuraParticle extends PitParticle {
	public TownAuraParticle() {
	}

	public TownAuraParticle(boolean accountForPitch, boolean accountForYaw) {
	...
	public void display(EntityPlayer entityPlayer, Location location, ParticleColor particleColor) {
		entityPlayer.playerConnection.sendPacket(new PacketPlayOutWorldParticles(
				EnumParticle.TOWN_AURA, true, (float) location.getX(), (float) location.getY(), (float) location.getZ(),
				0, 0, 0, 0, 0
		));
	}
}
