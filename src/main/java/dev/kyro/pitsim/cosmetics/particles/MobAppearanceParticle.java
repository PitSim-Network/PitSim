...
public class MobAppearanceParticle extends PitParticle {
	public MobAppearanceParticle(boolean accountForPitch, boolean accountForYaw) {
	...
	public void display(EntityPlayer entityPlayer, Location location, ParticleColor particleColor) {
		entityPlayer.playerConnection.sendPacket(new PacketPlayOutWorldParticles(
				EnumParticle.MOB_APPEARANCE, true, (float) location.getX(), (float) location.getY(), (float) location.getZ(),
				0, 0, 0, 0, 0
		));
	}
}
