...
public class SpellInstantParticle extends PitParticle {
	public SpellInstantParticle() {
	}

	public SpellInstantParticle(boolean accountForPitch, boolean accountForYaw) {
	...
	public void display(EntityPlayer entityPlayer, Location location, ParticleColor particleColor) {
		entityPlayer.playerConnection.sendPacket(new PacketPlayOutWorldParticles(
				EnumParticle.SPELL_INSTANT, true, (float) location.getX(), (float) location.getY(), (float) location.getZ(),
				0, 0, 0, 0, 0
		));
	}
}
