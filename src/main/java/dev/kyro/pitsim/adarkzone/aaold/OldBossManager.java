...
import dev.kyro.pitsim.adarkzone.aaold.slayers.*;
...
public class OldBossManager implements Listener {
	...
	private void spawnBoss(SubLevel level, Player player) throws Exception {
		switch(level.bossItem) {
			case ZOMBIE_FLESH:
				...
				break;
			case SKELETON_BONE:
				new SkeletonBossOld(player);
				break;
			case SPIDER_EYE:
//				new SpiderBoss(player);
				break;
			case CREEPER_POWDER:
				new ChargedCreeperBossOld(player);
				break;
			case CAVESPIDER_EYE:
				new CaveSpiderBossOld(player);
				break;
			case PIGMAN_PORK:
				new ZombiePigmanBossOld(player);
				break;
			case MAGMACUBE_CREAM:
				new MagmaCubeBossOld(player);
				break;
			case WITHER_SKELETON_SKULL:
				new WitherSkeletonBossOld(player);
				break;
			case GOLEM_INGOT:
				new IronGolemBossOld(player);
				break;
			case ENDERMAN_PEARL:
				new EndermanBossOld(player);
				break;
		}
	}
	...
	public void onVanillaAttack(EntityDamageByEntityEvent event) {
		...
		if(OldPitBoss.isPitBoss((Player) defender) && damager instanceof Arrow) {
			if(((Arrow) damager).getShooter() instanceof Player) {
				damager = (Entity) ((Arrow) damager).getShooter();

				if(OldPitBoss.isPitBoss((Player) damager)) {
					event.setCancelled(true);
				}
			}
		}
