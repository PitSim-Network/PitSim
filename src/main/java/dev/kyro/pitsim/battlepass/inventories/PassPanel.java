...
public class PassPanel extends AGUIPanel {
	...
	public PassPanel(AGUI gui) {
		...
		passInfo = new AItemStackBuilder(Material.BEACON)
				...
				.setLore(new ALoreBuilder(
						"&7Pass Level: " + (passData.getPremium() ? "&e&lPREMIUM" : "&7&lFREE"),
...
//			Set up the glass panes
			...
			if(PassManager.hasReward(PitSimPass.RewardType.PREMIUM, tier)) {
				...
				setLore(PitSimPass.RewardType.PREMIUM, itemStack, passData.getCompletedTiers() >= tier, hasClaimed, passData.getPremium());
			...
			if(PassManager.hasReward(PitSimPass.RewardType.FREE, tier)) {
				...
				setLore(PitSimPass.RewardType.FREE, itemStack, passData.getCompletedTiers() >= tier, hasClaimed, passData.getPremium());
		...
		if(slot == 13) {
			...
			if(PassManager.canClaimReward(pitPlayer, PitSimPass.RewardType.PREMIUM, clickedReward)) {
				...
				if(success) {
					...
					setLore(PitSimPass.RewardType.PREMIUM, itemStack, passData.getCompletedTiers() >= clickedReward, true, passData.getPremium());
					itemStack.setType(Material.INK_SACK);
					itemStack.setDurability((short) 8);
					itemStack.setAmount(1);
			...
			if(PassManager.canClaimReward(pitPlayer, PitSimPass.RewardType.FREE, clickedReward)) {
				...
				if(success) {
					...
					setLore(PitSimPass.RewardType.FREE, itemStack, passData.getCompletedTiers() >= clickedReward, true, passData.getPremium());
					itemStack.setType(Material.INK_SACK);
					itemStack.setDurability((short) 8);
					itemStack.setAmount(1);
					getInventory().setItem(slot, itemStack);
